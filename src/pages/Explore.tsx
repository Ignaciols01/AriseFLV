...
export default function Explore() {
  ...
  const fetchCatalog = async () => {
    ...
    const timeoutId = setTimeout(() => {
      isTimeout = true;
      setErrorMsg("La conexión está tardando demasiado. Si estás en una red restrictiva, el cortafuegos podría estar bloqueando Supabase, o la base de datos se está despertando.");
      setIsLoading(false);
    }, 6000); 
    ...
    <div className="min-h-screen bg-gradient-to-br from-red-950 to-[#2a0808] font-sans pb-12">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 relative z-10">
        ...
        <div className="flex items-center gap-3 mb-8">
          ...
          <h1 className="text-4xl font-black text-white tracking-tight uppercase">Catálogo Global</h1>
        </div>
        ...
        {isLoading ? (
          <div className="flex flex-col justify-center items-center py-24 gap-4">
            ...
            <p className="text-red-300 font-bold uppercase tracking-widest text-sm animate-pulse">Conectando con la bóveda...</p>
          </div>
        ) : errorMsg ? (
           <div className="flex flex-col items-center justify-center py-20 bg-red-950/50 rounded-2xl border-2 border-red-800 text-center px-4 shadow-xl">
            <AlertCircle className="w-16 h-16 text-red-500 mb-4" />
            <h3 className="text-xl font-black text-white uppercase tracking-wider mb-2">Conexión Interrumpida</h3>
            <p className="text-red-200 font-medium max-w-lg mb-8">{errorMsg}</p>
            <button 
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-6">
            {filteredCatalog.map((anime) => (
              <div key={anime.id} className="group rounded-xl bg-white shadow-xl overflow-hidden relative flex flex-col border border-red-100 hover:-translate-y-2 transition-all duration-300">
          ...
          <div className="flex flex-col items-center justify-center py-24 bg-red-950/50 rounded-2xl border-2 border-dashed border-red-800">
            <h3 className="text-xl font-black text-white uppercase tracking-wider">Sin resultados</h3>
          </div>
        )}
      </main>
    </div>
